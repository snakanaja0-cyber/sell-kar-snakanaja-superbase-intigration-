      ...
      <div className="max-w-4xl mx-auto">
        ...
        <div className="card-premium max-w-3xl mx-auto">
          <div className="space-y-8">
            {/* Text changed to darkText */}
            <h3 className={`text-2xl font-semibold text-center ${darkText} mb-8`}>
              What would you like to sell?
            </h3>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              {deviceTypes.map((device) => (
                <Link key={device.id} to={`/sell-${device.id}`}>
                  {/* Added royalBlueRing for hover effect */}
                  <Card className={`card-premium cursor-pointer group hover:scale-105 transition-all duration-300 ${royalBlueRing}`}>
                    ...
                    <div className="text-center">
                      ...
                      {/* Text changed to darkText */}
                      <p className={`text-sm ${darkText}`}>{device.description}</p>
                    </div>
                  </Card>
                </Link>
              ))}
